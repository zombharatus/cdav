...
class Dolibarr extends AbstractBackend implements SyncSupport
{
    ...
    protected function _toVCard($obj)
    {
        ...
        foreach ($soc_address as $kAddr => $vAddr)
            $soc_address[$kAddr] = trim(str_replace(array(
                "\r",
                "\t"
            ), ' ', str_replace("\n", ' | ', trim($vAddr))));
        $soc_address[] = '';
        ...
        foreach ($address as $kAddr => $vAddr) {
            $address[$kAddr] = trim(str_replace(array(
                "\r",
                "\t"
            ), ' ', str_replace("\n", ' | ', trim($vAddr))));
        }
        ...
        $carddata .= "REV;TZID=" . date_default_timezone_get() . ":" . strtr($obj->lastupd, array(
            " " => "T",
            ":" => "",
            "-" => ""
        )) . "\n";
        ...
        if (isset($vCard->PHOTO) && strpos(substr($vCard->PHOTO, 0, 10), '://') === false) // exist and not uri
{
        ...
        if (isset($vCard->BDAY) && ! empty($bday) && date("Y-m-d", strtotime(trim($bday))) == trim($bday))
